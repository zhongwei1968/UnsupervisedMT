...
class TrainerMT(MultiprocessingEventLoop):
    ...
    def discriminator_step(self):
        ...
        for lang_id, lang in enumerate(self.params.langs):
            ...
            with torch.no_grad():
                encoded.append(self.encoder(sent1.cuda() if self.params.cuda else sent1,
                                            len1, lang_id))
        ...
        self.dis_target = self.dis_target.contiguous().long().cuda() if self.params.cuda else \
            self.dis_target.contiguous().long()
    ...
    def lm_step(self, lang):
        ...
        sent1 = sent1.cuda() if self.params.cuda else sent1
    ...
    def otf_bt(self, batch, lambda_xe, backprop_temperature):
        ...
        if backprop_temperature == -1:
            ...
            # lang2 -> lang3
            bos = torch.cuda.FloatTensor(1, bs, n_words2).zero_() if self.params.cuda else \
                  torch.FloatTensor(1, bs, n_words2).zero_()
